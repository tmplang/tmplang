...
#else
// expected-error@+4 {{expected at least one clause on '#pragma omp declare mapper' directive}}
// expected-note@+3 {{'it' declared here}}
// expected-error@+2 {{only variable 'vvec' is allowed in map clauses of this 'omp declare mapper' directive}}
// expected-error@+1 {{incorrect map type modifier, expected one of: 'always', 'close', 'mapper', 'ompx_hold'}}
#pragma omp declare mapper(id2: struct vec vvec) map(iterator(it=0:vvec.len:2), tofrom:vvec.data[it])
#endif // OMP52
